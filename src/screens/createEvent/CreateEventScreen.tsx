...
//       useNativeDriver: true,
//     }).start();
//   }, 10); // slight delay so modal mounts before animation starts
// };
...
//               setEventData({ title: '', date: '', intent: '' });
//             },
//           },
//         ]
//       );
...
const CreateEventScreen: React.FC = () => {
  ...
  const handleCreateEvent = () => {
    if (validateForm()) {
      Alert.alert(
        'Event Created!',
        `"${eventData.title}" has been created successfully. Generate QR code now?`,
        [
          {
            text: 'Later',
            style: 'cancel',
          ...
          },
          {
            ...
            onPress: () => {
              console.log('Event Data:', eventData);
              // In real app: navigation.navigate('QRCodeScreen', { eventData, userData })
              Alert.alert('Navigation', 'Navigate to QR Code screen with event data');
              setEventData({ title: '', date: '', intent: '' });
            },
          },
        ]
